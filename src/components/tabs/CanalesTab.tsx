...
interface CanalesTabProps {
  data: Lead[];
}

export function CanalesTab({ data }: CanalesTabProps) {
  const canalesData = useMemo(() => {
    const map = new Map<string, any>();
    
    data.forEach(lead => {
      if (!map.has(lead.atribucion)) {
        map.set(lead.atribucion, { canal: lead.atribucion, leads: 0, ganados: 0, perdidos: 0, ingresos: 0 });
      }
    ...
    return Array.from(map.values()).map(c => ({
      ...
      ingresoPorLead: c.leads > 0 ? c.ingresos / c.leads : 0,
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Ingresos por Canal</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={canalesData} layout="vertical" margin={{ left: 50 }}>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Win Rate por Canal</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={canalesData} layout="vertical" margin={{ left: 50 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f3f4f6" />
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Rendimiento Detallado por Canal</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 text-gray-500 text-xs uppercase tracking-wider bg-gray-50">
                <th className="p-4 rounded-tl-lg">Canal</th>
                ...
                <th className="p-4 text-right">Ingreso/Lead</th>
                <th className="p-4 text-right rounded-tr-lg">Ticket Medio</th>
              </tr>
            </thead>
            <tbody>
              {canalesData.map((c, i) => (
                <tr key={c.canal} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="p-4 font-medium text-gray-900">{c.canal}</td>
                  ...
                  <td className="p-4 text-right text-gray-600">{c.ingresoPorLead.toLocaleString('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 })}</td>
